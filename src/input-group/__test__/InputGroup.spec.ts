...
import { InputGroup } from 'src/input-group/InputGroup'

describe('Label', () => {
  const p = React.createFactory('p')
  const F = React.createFactory<any>(InputGroup)
  ...
  it('should render correct className with different props', () => {
    const render = mount(F())
    expect(render.find('div').hasClass('input-group')).toBe(true)

    render.setProps({ size: 'large' })
    expect(render.find('div.input-group').hasClass('input-group-lg')).toBe(true)
    render.setProps({ size: 'small' })
    expect(render.find('div.input-group').hasClass('input-group-sm')).toBe(true)
  })
})