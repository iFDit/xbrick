...
import { InputGroupText } from 'src/input-group/InputGroupText'

describe('InputGroupText', () => {
  const p = React.createFactory('p')
  const F = React.createFactory<any>(InputGroupText)
  ...
  it('should render correct className with different props', () => {
    const render = mount(F())
    expect(render.find('div').hasClass('input-group-text')).toBe(true)
  })
})
