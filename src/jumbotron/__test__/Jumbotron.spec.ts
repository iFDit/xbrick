...
describe('Jumbotron', () => {
  ...
  const F = React.createFactory<any>(Jumbotron)
  ...
  it('should render correct className when set different props', () => {
    ...
    expect(render.find('div').hasClass('jumbotron')).toBe(true)

    render.setProps({ fluid: true })
    expect(render.find('div.jumbotron').hasClass('jumbotron-fluid')).toBe(true)
  })
})